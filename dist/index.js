...
async function run() {
	try {
		...
		try {
			...
			if (error.status === 404) core.setFailed(`Repository ${repoName} not found in organization`);
			else core.setFailed(`Failed to get repository info: ${error.message}`);
			return;
		}
		...
		const buildScripts = Object.entries(packageJson.scripts || {}).filter(([name]) => name.startsWith("build:")).map(([name, script]) => ({
			name,
			script
		}));
		...
		const getPackageVersion = (dir = process.cwd()) => {
			const packageJsonPath$1 = path.join(dir, "package.json");
			const packageJson$1 = JSON.parse(fs.readFileSync(packageJsonPath$1, "utf8"));
			return packageJson$1.version || "0.0.0";
		};
		const zipFiles = [];
		for (const { name, script } of buildScripts) {
			...
			const version$1 = getPackageVersion();
			const scriptPart = scriptName === "build" ? "" : `-${scriptName}`;
			const zipFileName = `${repoName.replace("/", "-")}${scriptPart}-BuildPackage-${version$1}.zip`;
